...
class UnsupervisedPart_CelebA(nn.Module):
    def __init__(self, num_part, d_model, pretrain, cfg):
        super(UnsupervisedPart_CelebA, self).__init__()
        ...
        self.ArcMarginProduct = ArcMarginProduct(d_model, self.num_part-self.background, self.num_part-self.background)
        ...
        self.Transformer = Get_vit(num_part, 128, 16, 256, 6, 8, 64)
    ...
    def Concentration_loss(self, pred, epsilon=2.0):
    ...
    def forward(self, input, input_pair=None, VGG=None, trainable=False):
        ...
        if trainable:
            ...
            concentration_loss_raw_pair, area_loss_raw_pair = self.Concentration_loss(output_distribution_raw_pair[:, :self.num_part-self.background, :, :])
            Highlevel_loss_pair_raw = self.high_level_loss(R_img, input, VGG)
            concentration_loss_pair_raw, area_loss_pair_raw = self.Concentration_loss(output_distribution_pair_raw[:, :self.num_part-self.background, :, :])
            ...
            semantic_loss = self.ArcMarginProduct(Global_Feature[:, :self.num_part-self.background, :]) \
                            + self.ArcMarginProduct(Global_Feature_pair[:, :self.num_part-self.background, :])
